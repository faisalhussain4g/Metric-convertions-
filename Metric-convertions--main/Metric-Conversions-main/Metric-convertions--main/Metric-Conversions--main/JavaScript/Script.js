...
function convertCurrencyOnly() {
    ...
    resultDiv.innerHTML =
        `${amount} ${fromSelect.value} = ${converted.toFixed(4)} ${toSelect.value}`;
}
...
// Initialize
populateCurrencies();   // pehle dropdown fill
fetchRates();           // phir API call


// Event listeners
amountInput.addEventListener('input', convertCurrencyOnly);
fromSelect.addEventListener('change', updateBaseAndConvert);
toSelect.addEventListener('change', convertCurrencyOnly);
